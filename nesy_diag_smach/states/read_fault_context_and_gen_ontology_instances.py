...
class ReadFaultContextAndGenOntologyInstances(smach.State):
    ...
    Based on the read information, ontology instances are generated, i.e., the diag-entity-specific instance data
    ...
    def __init__(self, data_accessor: DataAccessor, data_provider: DataProvider, kg_url: str, verbose: bool) -> None:
        ...
        smach.State.__init__(
            ...
            input_keys=[''],
            output_keys=['diag_entity_specific_instance_data']
        )
    ...
    def log_state_info(self) -> None:
        ...
        if self.verbose:
            ...
            print("\n\n############################################")
            print(
                "executing", colored("READ_FAULT_CONTEXT_AND_GEN_ONTOLOGY_INSTANCES", "yellow", "on_grey", ["bold"]),
                "state.."
            )
            print("############################################")
    ...
    @staticmethod
    def create_session_setup() -> None:
        """
        Creates the session setup.
        """
    ...
    def execute(self, userdata: smach.user_data.Remapper) -> str:
        ...
        self.instance_gen.extend_knowledge_graph_with_diag_entity_data(fault_context.diag_entity_id)

        self.create_tmp_file_for_unused_error_code_instances(fault_context)
        userdata.diag_entity_specific_instance_data = fault_context
